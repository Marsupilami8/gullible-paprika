...
.EXAMPLE
    VerifyImageHash.ps1 -TargetFolder "D:\IMAGES\TX1\" -OutputFolder "C:\Users\DFIR\Documents\Case 123\Image Verification Logs\"
...
.EXAMPLE
    VerifyImageHash.ps1 -OutputFolder "C:\Users\DFIR\Documents\Case 123\Image Verification Logs\"
...
.NOTES
    The FTK Imager command line tool (version 3.1.1) must be downloaded from Accessdata and installed to a folder of your choice (See link). It is reccomended that the path to the "ftkimager.exe" executable be placed in the system's PATH environment variable; otherwise, its explicit path must be added in this script. The FTK Imager tool does not work to verify images in AD1, L01, DD, or any other formats. The number of background jobs is limited to 20. When a max is achieved, no other jobs are started until a job is freed, after a timed waiting period. It is conceivable that this ceiling can be raised; however, no stress tests have been done. Use at your own risk. The script outputs the ftkimager.exe command line stderr messages to stdout, which will be captured in the log text file. This does not any impact results.
...
foreach($Image in $ForensicImages){
 ...
 if ($Image.Extension -IN ".e01x", ".ad1", ".l01", ".l01x", ".dd", ".001", ".zip") {
    ...
    $NonSupportMsg =  (Get-Date -Format s) + "`r`n$Image`r`nThe verification of this image type is unsupported. Please verify using an alternate method.`r`n" `        + "---------------------------------------------------------" + "`r`n" 
